...
void AXP192_get_mpp(void)
{
  ...
  dataBuffer.data.mpp_last_timeinfo = timeinfo;
  ...
  else
  {
    ...
    ESP_LOGI(TAG, "AXP: Get max power point");

    // get old value
    int old = pmu.getChargeControlCur();
    uint8_t max = 0;
    float power_old = 0;

    ty_mpp lt_mpp_values[13];

    for (int i = 0; i < 13; i++)
    {
      pmu.setChargeControlCur(i);
      delay(500);

      lt_mpp_values[i].pmu_charge_setting = i;
      lt_mpp_values[i].bus_voltage = pmu.getVbusVoltage() / 1000;
      lt_mpp_values[i].bat_charge_current = pmu.getBattChargeCurrent();
      lt_mpp_values[i].bat_charge_power = lt_mpp_values[i].bus_voltage * lt_mpp_values[i].bat_charge_current;

      if (lt_mpp_values[i].bat_charge_power > power_old)
      {
        max = i;
        dataBuffer.data.mpp_max_bat_charge_power = lt_mpp_values[i].bat_charge_power;
        dataBuffer.data.mpp_max_charge_setting = i;
      }
      power_old = lt_mpp_values[i].bat_charge_power;

      ESP_LOGI(TAG, "Limit %d : charging, %.2fV @ %.0fmAh --> %.0f mW", i, lt_mpp_values[i].bus_voltage, lt_mpp_values[i].bus_current, lt_mpp_values[i].bat_charge_power);
    }
    // Restore old value
    pmu.setChargeControlCur(old);
    memcpy(dataBuffer.data.mpp_values, lt_mpp_values, sizeof(lt_mpp_values));
